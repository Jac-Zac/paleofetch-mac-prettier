...
static int execute_command(const char *cmd, char *ref) {
  FILE *stdout_file = popen(cmd, "r");
  if (stdout_file) {
    char c;
    int i = 0;
    while ((c = fgetc(stdout_file)) != EOF && c != '\n' && i < 254) {
      ref[i] = c;
      i++;
    }
    ref[i] = '\0';
    int status = pclose(stdout_file);
    return (status == 0 && i > 0) ? 0 : -1;
  }
  return -1;
}
...
uint64_t get_mem_from_vm_stat() {
  ...
  kern_return_t ret;
  if ((ret = host_statistics64(myHost, HOST_VM_INFO64, (host_info64_t)&vm_stat,
                               &count) != KERN_SUCCESS)) {
  ...
  uint64_t total =
      (uint64_t)(vm_stat.compressor_page_count + vm_stat.wire_count +
                 vm_stat.active_count + vm_stat.speculative_count);
  total *= pagesize;
  total >>= 20;
  return total;
}
...
void get_ram_usage(char *ram_usage) {
  ...
  snprintf(ram_usage, BUFF_64, "%lluMB/%dMB %c%llu%s", used_memory,
           ram_size_short, '(',
           used_memory * 100 / (ram_size_short != 0 ? ram_size_short : 1),
           "%)");
}
#else
...
void get_gpu(char *gpu) {
  ...
  if (IOServiceGetMatchingServices(
...
#endif
#endif
          matchDict, &iterator) == kIOReturnSuccess) {
    ...
    while ((regEntry = IOIteratorNext(iterator))) {
      ...
      const void *GPUModel =
          CFDictionaryGetValue(serviceDictionary, CFSTR("model"));
...
void get_ip(char *local_ip) {
  ...
  if (execute_command(cmd, ip) == 0 && ip[0] != '\0') {
    strlcpy(local_ip, ip, BUFF_256);
  } else {
    strlcpy(local_ip, "Not connected to the internet", BUFF_256);
  }
}
